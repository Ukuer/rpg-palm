...
class RPGModel(BaseModel):
    ...
    def __init__(self, opt, id_model=None):
        ...
        if opt.isTrain:
            ...
            self.use_ID = self.opt.lambda_id > 0.0
    ...
    def set_input(self, input, test_mode=False):
    ...
    def backward_EG(self):
        ...
        if self.id_model is not None:
            if self.opt.lambda_id > 0.0:
                fake_B_encode_feature = self.id_model(transforms.functional.resize(self.fake_B_encoded * 0.5, [224, 224]))
                fake_B_random_feature = self.id_model(transforms.functional.resize(self.fake_B_random * 0.5, [224, 224]))
                self.loss_id = (1.0 - torch.cosine_similarity(fake_B_encode_feature,
                                                              fake_B_random_feature)) * self.opt.lambda_id

        self.loss_G = self.loss_G_GAN + self.loss_G_GAN2 + self.loss_G_L1 + self.loss_kl + self.loss_id
